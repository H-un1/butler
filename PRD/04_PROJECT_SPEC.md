...
| 영역 | 선택 | 이유 |
|------|------|------|
| 모바일 (점검자) | React Native (iOS + Android) | 현장 점검자(INSPECTOR) 전용. 카메라·체크리스트 등 모바일 네이티브 기능 필요. (임차인 RN 앱은 Phase 2) |
| 웹앱 (임대인 + 관리자) | React.js | **임대인(LANDLORD)과 관리자(ADMIN)가 같은 웹앱**을 공유하고 `role`로 화면·메뉴·API 권한을 분기. 별도 도메인/코드베이스 아님. |
...
```
CLIENT      점검자 모바일 (RN, iOS/Android) · 임대인+관리자 웹 (React, role 분기)
   │
...
butler/
├── apps/
│   ├── mobile/         # React Native — 점검자(INSPECTOR) 전용 (Phase 2: 임차인 추가)
│   └── web/            # React 웹앱 — 임대인(LANDLORD) + 관리자(ADMIN) 공유, role 분기
├── services/
...
- [ ] 역할(RBAC) 검증 없이 API를 열지 마 — 타 사용자가 본인 권한 밖 데이터(타인 물건·점검·정산 등)에 접근 불가. 임대인/관리자 공용 웹이라도 `role` 분기와 서버측 권한 검증을 둘 다 둔다.
- [ ] 테스트 없이 배포하지 마.
